...
test.skipIf(process.platform !== "darwin" || process.arch !== "arm64")(
  ...
  async () => {
    ...
    try {
      ...
      `;
      let reply: any;
      const worker = Bun.spawn([join(output, "Contents/MacOS/Luon Player"),
        "--luon-crawl-worker"], {
      ...
      try { await worker.exited; } finally { clearTimeout(timer); }
      ...
      const server = Bun.spawn([join(output, "Contents/MacOS/Luon Player"),
